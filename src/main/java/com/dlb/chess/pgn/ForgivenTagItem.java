...
/**
 * One tag-level deviation surfaced by the lenient PGN parser. The parse model preserves input as given; this
 * record is the diagnostic channel telling consumers which deviations the parser tolerated.
 *
...
public record ForgivenTagItem(ForgivenTagItemCode code, String tagName, String detail) {
  ...
  /**
   * Shared empty list for the "no tag deviations forgiven" case. Centralised here so the {@code @NonNull}
   * suppression on Guava's {@code ImmutableList.of()} (which JDT can't statically prove is non-null with non-null
   * elements) lives in one place rather than at every caller.
   */
