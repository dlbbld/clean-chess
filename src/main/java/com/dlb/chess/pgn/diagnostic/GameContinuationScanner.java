...
/**
 * Standalone scanner that detects whether a sequence of halfmoves continued past one of the two FIDE-automatic,
 * path-dependent terminations: fivefold repetition or the 75-move rule.
 *
 * <p>
 * This utility is intended for corpus-mining workflows: scanning historical PGN databases to identify games whose
 * recorded halfmove sequence continued beyond the point at which the game would have ended automatically under current
 ...
 *
 * <p>
 * Position-only terminations (checkmate, stalemate, insufficient material) are not addressed here because they are not
 * path-dependent and are queryable directly from the current position.
 */
public final class GameContinuationScanner {
  ...
  /**
   * Returns {@code true} iff the supplied halfmove sequence reached a fivefold repetition at some halfmove and
   * continued past it (i.e., the fivefold-repeated halfmove is not the last in the sequence).
   */
  ...
  /**
   * Returns {@code true} iff the supplied no progress sequence list contains a sequence whose length strictly exceeds
   * the 75-move rule threshold (150 halfmoves), indicating that play continued past the automatic 75-move termination.
   */
  public static boolean isContinuedOverSeventyFiveMove(List<List<NoProgressHalfMove>> noProgressMoveListList) {
