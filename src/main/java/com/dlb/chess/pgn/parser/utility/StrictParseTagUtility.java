...
public abstract class StrictParseTagUtility extends AbstractParseTagUtility {
  ...
  private static void validateTagNameSquareBrackets(String tagLine) {
    if (!tagLine.startsWith(LEFT_SQUARE_BRACKET)) {
      throw new StrictPgnParserValidationException(
          StrictPgnParserValidationProblem.TAG_FORMAT_NOT_STARTING_WITH_LEFT_SQUARE_BRACKET, SanValidationProblem.NONE,
          "The tag line must start with the left square bracket " + LEFT_SQUARE_BRACKET);
    }
    if (!tagLine.endsWith(RIGHT_SQUARE_BRACKET)) {
      throw new StrictPgnParserValidationException(
          StrictPgnParserValidationProblem.TAG_FORMAT_NOT_ENDING_WITH_RIGHT_SQUARE_BRACKET, SanValidationProblem.NONE,
          "The tag line must end with the right square bracket " + RIGHT_SQUARE_BRACKET);
    }
    if (tagLine.startsWith(LEFT_SQUARE_BRACKET + " ")) {
      throw new StrictPgnParserValidationException(
          StrictPgnParserValidationProblem.TAG_FORMAT_LEFT_SQUARE_BRACKET_FOLLOWED_BY_SPACE, SanValidationProblem.NONE,
  ...
  private static void validateTagNameFirstCharacter(String tagLine) {
    ...
    // check all occurance
    if (!matcher.matches()) {
      throw new StrictPgnParserValidationException(StrictPgnParserValidationProblem.TAG_NAME_FIRST_CHARACTER,
  ...
  private static void validateTagNameLength(String tagName) throws StrictPgnParserValidationException {
    if (tagName.length() > MAX_TAG_NAME_LENGTH) {
      throw new StrictPgnParserValidationException(StrictPgnParserValidationProblem.TAG_NAME_EXCEEDS_MAXIMUM_LENGTH,
  ...
  private static boolean validateTagFormat(String tagLine) throws StrictPgnParserValidationException {
    ...
    throw new StrictPgnParserValidationException(StrictPgnParserValidationProblem.TAG_FORMAT_INVALID,
        SanValidationProblem.NONE,
        "A tag line with an invalid format was found. The expected format is exactly like: [White \"Alpha Zero\"]. The tag line is \""
            + tagLine + "\"");
  }

  private static void validateTagUniqueNames(List<Tag> tagList) throws StrictPgnParserValidationException {
  ...
  private static void validateSevenTagRoster(List<Tag> tagList) throws StrictPgnParserValidationException {
  ...
  public static List<Tag> validateTagList(List<String> fileLines) throws StrictPgnParserValidationException {
