...
/**
 * Marks PGN files in the test corpus whose recorded halfmove sequence continues past one of the five FIDE-automatic
 * terminations (checkmate, stalemate, mutual insufficient material, fivefold repetition, 75-move rule).
 *
 * <p>
 * Under the strict-game invariant such PGNs cannot be fully replayed by the standard parsers —
 * {@code Board.performMove} rejects the first move past the terminating halfmove with a {@code GAME_ALREADY_ENDED}
 * diagnostic. The PGN files themselves are kept on disk because they are deliberately crafted edge-case test fixtures
 * (carefully constructed positions exercising intervening repetitions, sixfold positions, FENs at the 75-move
 * threshold, dead-position captures, and so on). Their test role flips from "fully replay and verify analysis" to
 * "verify that strict-pipeline import correctly rejects continuation."
 *
 * <p>
 * Iterating tests that call {@code Analyzer.calculateAnalysis(...)} or otherwise replay the full halfmove sequence skip
 * files identified by this class. A dedicated rejection test ({@code TestPgnPlaysBeyondTerminationRejection}) iterates
 * them separately and asserts {@code GAME_ALREADY_ENDED} is thrown during import.
 *
 * <h2>Why an explicit registry, not a substring matcher</h2>
 *
 * <p>
 * The classification is an <strong>explicit, hard-coded set of basenames</strong> ({@link #PGN_FILE_NAMES}), not a
 * substring or pattern match. Substring matching is fragile: if a PGN is renamed and no longer matches the historical
 * pattern, the old logic silently stops classifying it, and tests that should skip "plays beyond" files start replaying
 * them (or vice versa, the rejection test stops exercising it). With an explicit registry, such a drift is detected
 * immediately by {@link com.dlb.chess.test.pgntest.TestPgnPlaysBeyondTerminationRegistry} which walks the corpus on
 * every test run and fails if any registered name no longer exists on disk.
 */
public final class PgnPlaysBeyondTermination {

  /**
   * The complete, explicit list of PGN basenames in the test corpus whose recorded sequence continues past a
   * FIDE-automatic termination.
   *
   * <p>
   * Categories represented (kept in line-grouped form for review):
   * <ul>
   * <li>basic/fivefold — fivefold repetition continuation</li>
   * <li>basic/seventyFive — 75-move rule continuation</li>
   * <li>basic/intervening — outer-claim / inner-automatic interactions (encapsulating-fivefold, interlocked-fivefold,
   * and the asymmetric 07-interlocked-threefold case)</li>
   ...
   *
   * <p>
   * Adding or renaming such a fixture in the corpus must be paired with a corresponding edit here. The registry
   * validator catches missing entries; coverage of newly added "plays beyond" files surfaces naturally because the
   * rejection test will fail to assert rejection on them.
   */
  public static final Set<String> PGN_FILE_NAMES = NonNullWrapperCommon.setOf(
      ...
      "fivefold_beyond_savchenko_yu_y2017.pgn", "fivefold_beyond_wang_yu_2017.pgn",
      "fivefold_beyond_yu_alekseenko_2018.pgn", "seventy_five_beyond_anton_guijarro_antipov_2015.pgn",
      "seventy_five_beyond_aronian_navara_2017.pgn", "seventy_five_beyond_cheparinov_jones_2019.pgn",
      "seventy_five_beyond_moiseenko_radjabov_2016.pgn", "seventy_five_beyond_onischuk_guseinov_2014.pgn");

  private PgnPlaysBeyondTermination() {
  }

  /**
   * Returns {@code true} iff the given PGN basename is registered as recording halfmoves past a FIDE automatic
   * termination, i.e. cannot be fully replayed under the strict-game invariant.
   *
   * <p>
   * This is an exact basename match against {@link #PGN_FILE_NAMES}. There is no substring or pattern matching — see
  ...
  /**
   * Walks the PGN test corpus rooted at {@code corpusRoot} and returns the registered basenames that are NOT present on
   * disk. An empty result means the registry is in sync with the corpus; a non-empty result means one or more
   * registered files have been renamed or removed and the registry must be updated.
   *
   * <p>
   * The companion test {@link com.dlb.chess.test.pgntest.TestPgnPlaysBeyondTerminationRegistry} calls this method
   * against the live corpus and fails the build when the result is non-empty.
   */
  // The @SuppressWarnings("null") on this method covers the unsafe-null-conversion warning when
  // passing a SimpleFileVisitor<@NonNull Path> (made @NonNull via the package-level
  // @NonNullByDefault) to Files.walkFileTree, which expects a less-annotated FileVisitor<? super
  // Path>. The inner visitFile override carries its own @SuppressWarnings to address the
  // "illegal redefinition of parameter" diagnostic — JDT flags that the override declares
  // implicitly @NonNull parameters whereas the parent SimpleFileVisitor.visitFile declares them
  // unconstrained.
