...
import com.dlb.chess.board.InsufficientMaterialCalculator;
...
class TestInsufficientMaterial implements EnumConstants {
  ...
  private static boolean calculateIsInsufficientMaterial(StaticPosition staticPosition) {
    ...
    // KNvK, KvKN
    if (InsufficientMaterialCalculator.calculateIsKingAndKnightOnly(WHITE, staticPosition)
        && MaterialUtility.calculateIsKingOnly(BLACK, staticPosition)
        || MaterialUtility.calculateIsKingOnly(WHITE, staticPosition)
            && InsufficientMaterialCalculator.calculateIsKingAndKnightOnly(BLACK, staticPosition)) {
      return true;
    }

    // K(B^lightSquares)*vK(B^lightSquares)*, K(B^darkSquares)*vK(B^darkSquares)* (includes KvK)
    if (InsufficientMaterialCalculator.calculateHasZeroOrMultipleLightSquareBishopOnly(WHITE, staticPosition)
        && InsufficientMaterialCalculator.calculateHasZeroOrMultipleLightSquareBishopOnly(BLACK, staticPosition)
        || InsufficientMaterialCalculator.calculateHasZeroOrMultipleDarkSquareBishopOnly(WHITE, staticPosition)
            && InsufficientMaterialCalculator.calculateHasZeroOrMultipleDarkSquareBishopOnly(BLACK, staticPosition)) {
      return true;
    }
