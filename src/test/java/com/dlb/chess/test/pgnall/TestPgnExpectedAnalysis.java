 ...
 * {@code IS_CHECK_ONLY_PGN_TEST}, {@code IS_CHECK_FROM_PGN_FILE_NAME}) let a developer narrow the
 * test scope during local debugging, but {@link #IS_DISABLED} prevents the entire {@link #test}
 * method from running by default. Workflow:
 * <ol>
 ...
 *
 * <p>
 * The {@link #testIsDisabledMustBeTrueOnCommit} guard test asserts the constant is {@code true} on
 * commit, so accidentally committing it as {@code false} fails the build — turning the discipline
 * into a CI-checked invariant rather than relying on developer vigilance.
 */
class TestPgnExpectedAnalysis extends AbstractPgnTest {
  ...
  /**
   * Lock-down constant. MUST be {@code true} on commit. Flip locally to {@code false} for
   * debugging only and restore before committing — the guard test
   * {@link #testIsDisabledMustBeTrueOnCommit} fails the build if this is committed as
   * {@code false}.
   */
  ...
  /**
   * Guard: fails the build if {@link #IS_DISABLED} is committed as {@code false}. When the
   * developer is locally debugging with {@code IS_DISABLED = false}, this test fails (expected
   * locally), reminding them to restore it before committing.
   */
  @SuppressWarnings("static-method")
  @Test
  void testIsDisabledMustBeTrueOnCommit() {
    assertTrue(IS_DISABLED,
        "TestPgnExpectedAnalysis.IS_DISABLED must be true on commit. "
