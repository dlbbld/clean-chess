...
class TestPerformMoveForSan implements EnumConstants {
  ...
  void testCalculateMoveSpecificationForSan() {
    ...
    {
      final MoveSpecification parsedMove = checkValid(board, "e4", new MoveSpecification(WHITE, E2, E4));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "d5", new MoveSpecification(BLACK, D7, D5));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "exd5", new MoveSpecification(WHITE, E4, D5));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "e5", new MoveSpecification(BLACK, E7, E5));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "dxe6", new MoveSpecification(WHITE, D5, E6));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "h6", new MoveSpecification(BLACK, H7, H6));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "exf7+", new MoveSpecification(WHITE, E6, F7));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "Ke7", new MoveSpecification(BLACK, E8, E7));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "fxg8=Q",
          new MoveSpecification(WHITE, F7, G8, PromotionPieceType.QUEEN));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "h5", new MoveSpecification(BLACK, H6, H5));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "Qxh8", new MoveSpecification(WHITE, G8, H8));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "h4", new MoveSpecification(BLACK, H5, H4));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "g4", new MoveSpecification(WHITE, G2, G4));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "hxg3", new MoveSpecification(BLACK, H4, G3));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "a4", new MoveSpecification(WHITE, A2, A4));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "gxh2", new MoveSpecification(BLACK, G3, H2));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "a5", new MoveSpecification(WHITE, A4, A5));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "hxg1=Q",
          new MoveSpecification(BLACK, H2, G1, PromotionPieceType.QUEEN));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "a6", new MoveSpecification(WHITE, A5, A6));
      board.performMove(parsedMove);
    }

    {
      final MoveSpecification parsedMove = checkValid(board, "Qxh1", new MoveSpecification(BLACK, G1, H1));
      board.performMove(parsedMove);
    }
  ...
  private static MoveSpecification checkValid(ApiBoard board, String san, MoveSpecification expectedMove) {
